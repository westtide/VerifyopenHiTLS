...
/*@
  ...
  // 2 确定cnt == 0/1 ==? （求解器的压力）
  // 3 block 不确定的时候如何操作
  // 4. 一致性：另外写个协议的logic，再去证明一致
  // 5. sm4的模式？
  // 其中 XTS 模式为磁盘加密设计，使⽤了；ECB 模式为每个 16 字节明⽂块单独加密； CFB 模式⽤于加密流式数据；CTR 模式⽤于⾼速通信加密。以上都调⽤了arm汇编实现， 因此⽆法使⽤ Frama-C 验证。？模式也有C实现的:仓库的crypto/modes/include/crypt_modes_ecb.h
  // 方案的表述、思路、内容
  // rte mem安全spec
  // 8月底例会：
